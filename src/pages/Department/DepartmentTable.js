...
export default function DepartmentTable() {
  ...
  const getData = async (page) => {
    try {
      ...
      const res = await axios.get(
        `https://${params}.almnew.online/api/dept-all?page=${page}`, // Increment the page by 1

        config
      );
      // console.log(res);
      if (res.status === 200) {
        setData(res.data.departments.data);
        setTotalPages(res.data.departments.last_page);
      } else {
        // console.log(res);
      }
    } catch (error) {
  ...
  const searchData = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        `https://${params}.almnew.online/api/dept-search`,
        data,
        config
      );
      console.log(res);
  ...
  const create = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        `https://${params}.almnew.online/api/dept-create`,
        data,
  ...
  const data = {
    ...
    building_code: selectOneBuild?.code
      ? selectOneBuild?.code
      : editRow.build.code,
    level_code: selectOneLevel?.code ? selectOneLevel.code : editRow.level.code,
    // image,
  };
    // console.log(data);
    try {
      ...
      const res = await axios.post(
        `https://${params}.almnew.online/api/dept-update/` + editRow.id,
        data,
  ...
  const deleteHandle = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `https://${params}.almnew.online/api/dept-destroy/` + editRow.id,
  ...
  const saveProduct = async () => {
    ...
    if (uploadExcel) {
      try {
        ...
        const res = await axios.post(
          `https://${params}.almnew.online/api/dept-import`,
          { file: uploadExcel },
  ...
  const deleteMultipleHandle = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        `https://${params}.almnew.online/api/dept-deleteMultiple`,
        { id: idArray },
        config
      );
      // console.log(res);
  ...
  useEffect(() => {
    ...
    fetch(`https://${params}.almnew.online/api/departments-export`, {
      method: "GET",
      headers: {
        Authorization: `Bearer ${user.token}`,
      },
  ...
  useEffect(() => {
    ...
    fetch(`https://${params}.almnew.online/api/pdf-dept-convert`, {
      method: "GET",
      headers: {
        Authorization: `Bearer ${user.token}`,
      },
  ...
  return (
    <div className="p-2">
      ...
      <Dialog
        ...
        header={editRow ? "Edit Department" : "Add Department"}
        modal
        className="p-fluid"
        footer={
          <React.Fragment>
